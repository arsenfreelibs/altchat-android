...
public class AppUpdate {
  ...
  private static final long CHECK_INTERVAL_MS = 60 * 60 * 1000L; // 1h
  ...
  // Alt Chat: upstream's self-updater is disabled. It reads app-version info from relay IMAP
  // metadata that describes upstream Delta Chat releases, and Alt Chat ships its own update
  // flow (util/update/AppUpdateChecker: Play In-App Update on gplay, F-Droid on foss).
  private static final boolean SELF_UPDATE_SUPPORTED = false;

  public static boolean isSelfUpdateEnabled(Context context) {
    if (!SELF_UPDATE_SUPPORTED) return false;
