...
/**
 * Central service class orchestrating Alt Platform API calls with DC core operations. Not an
 * Android Service — plain Java class, instantiated with ApplicationContext. All methods are
...
public class AltPlatformService {
  ...
  /**
   * Registers a user on Alt Platform. Collects all addrs from rpc.listTransports(), exports DC
   * keys, encrypts private key, and calls /v1/users/register.
   */
  public RegisterResult register(
      String username, String email, String displayName, String recoveryPassword) {
    ...
    try {
      ...
      Log.d(
          TAG,
          "register() privateKey len="
              + (privateKeyArmored != null ? privateKeyArmored.length() : -1));
    } catch (Exception e) {
    ...
    RegisterRequest req =
        new RegisterRequest(
            username, email, addrs, displayName, publicKey, fingerprint, encryptedPrivateKey);
    Log.d(TAG, "register() calling API addrs=" + addrs + " fingerprint=" + fingerprint);
    Log.d(
        TAG,
        "register() publicKey(first80)="
            + publicKey.substring(0, Math.min(80, publicKey.length())));
    Log.d(TAG, "register() encryptedPrivateKey len=" + encryptedPrivateKey.length());
    Log.d(
        TAG, "register() username=" + username + " email=" + email + " displayName=" + displayName);
    AltApiResponse<Void> resp = api.register(req);
    Log.d(
        TAG, "register() API response httpCode=" + resp.httpCode + " errorCode=" + resp.errorCode);
  ...
  /**
   * Adds a DC contact from an Alt Platform profile. Creates via createContact (reliable), then
   * imports PGP key via gossip vCard if available.
   *
    ...
    try {
      ...
      if (profile.public_key != null && !profile.public_key.isEmpty()) {
        try {
          ...
          Log.w(
              TAG,
              "addContactFromAlt: importVcardContents failed: "
                  + e.getMessage()
                  + ", falling back");
        }
      }
   ...
   * Generates username from displayName, generates recovery password, encrypts private key. Saves
