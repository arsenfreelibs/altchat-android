...
/**
 * Bridges alt.chat call events to the Android Telecom stack. Registers a self-managed {@link
 * PhoneAccount} so that the OS is aware of VoIP calls (required for full-screen intent privileges
 * on locked screens).
 *
 * <p>Requires {@link android.Manifest.permission#MANAGE_OWN_CALLS} (normal permission, granted
 * automatically when declared in the manifest) and API level ≥ 26. All public methods are safe to
 * call on any thread and on all API levels; calls on API < 26 are silently ignored.
 */
public class TelecomHelper {
  ...
  public void registerPhoneAccount() {
    ...
    try {
      ...
      PhoneAccount account =
          PhoneAccount.builder(phoneAccountHandle(), context.getString(R.string.app_name))
              .setCapabilities(
  ...
  /**
   * Called when {@code DC_EVENT_INCOMING_CALL} fires. Informs Telecom of the new incoming call so
   * the OS has visibility into the active VoIP session (required for full-screen intent
   * privileges). Telecom calls back into {@link AltChatConnectionService} to create the {@link
   * AltChatConnection}.
   */
    ...
    Util.runOnAnyBackgroundThread(
        () -> {
          try {
            ...
            // Pass the address so Telecom can identify the call; mailto: is the natural scheme for
            // email-based VoIP.
  ...
  private PhoneAccountHandle phoneAccountHandle() {
    return new PhoneAccountHandle(
        new ComponentName(context, AltChatConnectionService.class), "altchat");
  }
