...
/**
 * Foreground service (phoneCall) that keeps the process alive during an incoming call so Android
 * does not kill the app.
 *
 * <p>Lifecycle: - {@link org.thoughtcrime.securesms.notifications.NotificationCenter#notifyCall}
 * starts it - {@link
 * org.thoughtcrime.securesms.notifications.NotificationCenter#removeCallNotification} stops it
 *
 * <p>Two-phase notification: 1. {@code onCreate()} posts a silent placeholder immediately to
 * satisfy the 5-second FGS rule. 2. {@code onStartCommand()} builds the full CallStyle notification
 * on a background thread and replaces the placeholder via a second {@code startForeground()} call —
 * same ID, one slot.
 */
