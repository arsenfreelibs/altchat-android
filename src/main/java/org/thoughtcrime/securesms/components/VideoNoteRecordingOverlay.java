...
public class VideoNoteRecordingOverlay extends FrameLayout {
  ...
  public VideoNoteRecordingOverlay(@NonNull Context context) {
    ...
    timerView = findViewById(R.id.video_note_timer);
  ...
  public void updateTimer(long elapsedMs) {
    long seconds = TimeUnit.MILLISECONDS.toSeconds(elapsedMs);
    long tenths = (elapsedMs / 100) % 10;
    timerView.setText(String.format(Locale.US, "%d:%02d.%d", seconds / 60, seconds % 60, tenths));
  }
